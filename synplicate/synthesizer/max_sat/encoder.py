# synplicate/synthesizer/max_sat/encoder.py
import os
...
def encode(encoder, output_dir_path,samples,num_of_feature_nodes,feature_partition,feature_weights,label_partition,feature_defs,lower_bound, upper_bound, precision, weights_map, file_name):
    ...
    if True:
        # phi_non_repeatability
        encoder.phi_non_repeat(sat_file, num_of_feature_nodes, feature_partition)
        sat_file.write("\n")

        # phi_region
        encoder.phi_region(sat_file,num_of_feature_nodes,feature_partition,feature_weights,lower_bound,upper_bound,precision, len(samples))


        sat_file.write("FORMULA :=  phi_E & phi_sim & phi_corr & phi_expl & phi_non_repeatability & phi_region;\n")
    ...
    soft_vars, corr_soft_vars, expl_soft_vars, corr_vars = encoder.extract_soft_variables(dimacs_path,feature_weights,num_of_feature_nodes, len(samples))#TODO extract should only take dimacs path. Every other info should be in encoder
