...
def encode(encoder, output_dir_path,samples,num_of_feature_nodes,feature_partition,feature_weights,label_partition,feature_defs,
lower_bound, upper_bound, precision, weights_map, file_name):
    ...
    # phi_region
    experiment_dir = os.path.dirname(os.path.abspath(output_dir_path))
    ...
    soft_vars, corr_soft_vars, expl_soft_vars, corr_vars = encoder.extract_soft_variables(
        dimacs_path,
        feature_weights,
        num_of_feature_nodes,
        feature_partition,
        experiment_dir,
    )#TODO extract should only take dimacs path. Every other info should be in encoder
