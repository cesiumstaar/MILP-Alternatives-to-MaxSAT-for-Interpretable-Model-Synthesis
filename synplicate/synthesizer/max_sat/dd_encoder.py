# synplicate/synthesizer/max_sat/dd_encoder.py
import math
...
def phi_corr(sat_file,samples):
        ...
        #tried this - but gives unsat, I think the corr clauses are being added somewhere else
        # sat_file.write("phi_corr := T")
        # for s in range(len(samples)):
        #     sat_file.write(f" & match_0_{s}")
        # sat_file.write(";\n")
...
def phi_non_repeat(sat_file,num_of_feature_nodes,feature_partition):
    ...
    sat_file.write("phi_reachability :=\n")
    sat_file.write("T")
    for node in range(num_of_feature_nodes):
        for nodep in range(node+1, num_of_feature_nodes):
            for bucket in range(max_bucket):
                sat_file.write(" &\n")
                sat_file.write("(")
                sat_file.write(f"!tau_{node:d}_{bucket:d}_{nodep:d} | reach_{node:d}_{nodep:d}")
                sat_file.write(")")

    for node in range(num_of_feature_nodes):
        for mid_node in range(node+1, num_of_feature_nodes-1):
            for nodep in range(mid_node+1, num_of_feature_nodes):
                sat_file.write(" &\n")
                sat_file.write("(")
                sat_file.write(f"!reach_{node:d}_{mid_node:d} | !reach_{mid_node:d}_{nodep:d} | reach_{node:d}_{nodep:d}")
                sat_file.write(")")

    sat_file.write(";\n\n")

    sat_file.write("phi_non_repeat :=\n")
    sat_file.write("T")
    for node in range(num_of_feature_nodes):
        for nodep in range(node+1, num_of_feature_nodes):
            for feature in feature_partition.keys():
                sat_file.write(" &\n")
                sat_file.write("(")
                sat_file.write(f"!lamp_{node:d}_{feature} | !reach_{node:d}_{nodep:d} | !lamp_{nodep:d}_{feature}")
                sat_file.write(")")

    sat_file.write(";\n\n")
    sat_file.write("phi_non_repeatability := phi_reachability & phi_non_repeat;\n\n")

def estimate_max_explanation_score(feature_weights, num_of_feature_nodes):

    if num_of_feature_nodes <= 0:
        return 1

    if feature_weights:
        max_feature_weight = max(feature_weights.values())
    else:
        max_feature_weight = 1

    max_feature_weight = max(1, max_feature_weight)
    max_score = num_of_feature_nodes + num_of_feature_nodes * max_feature_weight
    return max(1, max_score)


def compute_normalizied_weights(feature_weights,num_of_feature_nodes, num_samples):
    ...
    if feature_weights:
        ...
        mean_weight = sum(feature_weights.values())/len(feature_weights) if feature_weights else 1
    else:
        maximum_feature_weight = 1


    for node in range(num_of_feature_nodes):
        # weights_map[f"used_node_{node:d}"] = (maximum_feature_weight+1)     
        # weights_map[f"used_node_{node:d}"] = maximum_feature_weight+0.1*mean_weight  
        weights_map[f"used_node_{node:d}"] = mean_weight    


    max_expl_score = estimate_max_explanation_score(feature_weights, num_of_feature_nodes)
    print(f"|---Estimated maximum explanation score: {max_expl_score}")
    for key in weights_map.keys():
        weights_map[key] = max(1, int(weights_map[key]/max_expl_score*num_samples))
        # weights_map[key] = int(weights_map[key]/max_expl_score*num_samples)
        # print(f"|---Weight for {key}: {weights_map[key]}")
        # weights_map[key] = int(weights_map[key]/34*100)

    return weights_map


def phi_region(sat_file,num_of_feature_nodes,feature_partition,feature_weights,lower_bound,upper_bound, precision, num_samples):

    weights_map = compute_normalizied_weights(feature_weights,num_of_feature_nodes, num_samples)
...
def extract_soft_variables(dimacs,feature_weights,num_of_feature_nodes, num_samples):
    ...
    corr_vars = []
    text_file = open(dimacs)
    # sum_samples_weight = 0
    

    weights_map = compute_normalizied_weights(feature_weights,num_of_feature_nodes, num_samples)
    max_expl_weight = max(weights_map.values()) if weights_map else 1
    max_expl_weight = max(1, max_expl_weight)
    max_expl_score = estimate_max_explanation_score(feature_weights, num_of_feature_nodes)
    for line in text_file.readlines():
        if not line.startswith("c"):
            break
        ...
        if ("match_0" in line or " lamp_" in line or "used_node_" in line):
            ...
            if current_var in weights_map.keys():
                weight = max(1, int(weights_map[current_var]/corr_factor))
                ...
                weight = max_expl_score
                # weight = 1
                # sum_samples_weight +=1
                if((current_num,weight) not in soft_vars):
                        soft_vars.append((current_num,weight))
                        corr_soft_vars.append((current_num,weight))
                corr_vars.append((current_num,weight))

    # print(f"|------------sample weights sum: {sum_samples_weight}, {num_samples}")
            # reached_num = False
            # finished_var = False
            # for letter in line:
            #     if letter=='\n':
